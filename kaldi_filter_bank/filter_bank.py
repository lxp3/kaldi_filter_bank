...
class Filterbank(nn.Module):
    ...
    def __init__(
        ...
        if log_energy_floor > 0:
            self.register_buffer('_log_energy_floor', torch.tensor(log_energy_floor, dtype=torch.float32))
    ...
    def extract_frames_as_strided(self, waveforms: torch.Tensor) -> torch.Tensor:
        ...
        # Build index matrix
        frame_indices = torch.arange(self.win_length, device=waveforms.device).unsqueeze(0)  # [1, win_length]
        start_indices = torch.arange(num_frames, device=waveforms.device).unsqueeze(1) * self.hop_length  # [num_frames, 1]
        indices = frame_indices + start_indices  # [num_frames, win_length]
        
        # Expand to batch dimension
        indices = indices.unsqueeze(0).expand(batch_size, -1, -1)  # [B, num_frames, win_length]
        
        # Extract frames using gather
        frames = torch.gather(
            waveforms.unsqueeze(1).expand(-1, num_frames, -1), 
            2, 
            indices
    ...
    def preemphasize(self, frames: torch.Tensor) -> torch.Tensor:
        ...
        preemph_frames[..., 1:] = frames[..., 1:] - self.preemph_coeff * frames[..., :-1]
    ...
    def compute_power_spectrum_fft(self, windowed: torch.Tensor) -> torch.Tensor:
        ...
        magnitude_spec = magnitude_spec[..., :-1]   # [B, num_frames, n_stft] remove Nyquist
    ...
    def compute_power_spectrum_dft(self, windowed: torch.Tensor) -> torch.Tensor:
        ...
        spec_real = torch.matmul(windowed, self.dft_matrix_real)  # [B, num_frames, n_stft]
        spec_imag = torch.matmul(windowed, self.dft_matrix_imag)  # [B, num_frames, n_stft]
    ...
    def compute_power_spectrum(self, windowed: torch.Tensor, use_dft_matrix: bool = False) -> torch.Tensor:
        """
    ...
    def forward(self, waveforms: torch.Tensor) -> torch.Tensor:
        ...
        """
        if waveforms.dim() != 2:
            raise ValueError(f"Expected 2D tensor (batch, samples), got {waveforms.shape}")
        ...
        power_spec = self.compute_power_spectrum(windowed, use_dft_matrix=self.onnx_compatible)
